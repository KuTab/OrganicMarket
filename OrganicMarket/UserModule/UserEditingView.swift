    ...
    var body: some View {
        ScrollView {
            VStack {
                ...
                TextEditor(text: $viewModel.user.description)
                    .frame(maxWidth: .infinity, minHeight: 200)
                    .shadow(radius: 0.5)
                    .padding()
...
struct UserEditingView_Previews: PreviewProvider {
    @StateObject static var viewModel: UserEditingViewModel = .init(user: seedUser, updatePublisher: nil, errorPublisher: nil)
